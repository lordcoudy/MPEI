...
def butov(x, size) -> int:
    if x <= 0:
        return int(abs(x))
    elif x > size-1:
        return int(size-1 - x%size)
    return int(x)
...
h = numpy.array([[-1/8, -1/8, -1/8], [-1/8, 1, -1/8], [-1/8, -1/8, -1/8]])
        ...
        for ii in range(-M,M+1):
            for jj in range(-M,M+1):
                if ((i+ii)>(-1))&((j+jj)>(-1))&((i+ii)<(height))&((j+jj)<(width)):
                    ...
                    pix=pix+float(im[(butov(j+jj, width)),(butov(i+ii, height))])*h[jj+M,ii+M]
