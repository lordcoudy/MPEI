...
V = 20          # смещение графика по оси Y
...
AN = 1         # переменная для дальнейшего добавления шума (случайного смещения точек)
...
print(responses_type[numberOfGraphForShowing])
...
param_grid = {"max_iter": [1000], "activation": ["tanh"], "alpha": [0.65],
              "hidden_layer_sizes": [2], "solver": ["lbfgs"], "verbose": [True],}
...
for i in range(0, countOfTestSet):
    ...
    if Q <= 0.5:
        ...
        for j in range(0, countOfPoints):
            ...
            test_responses[i, j] += random.random() * 2 * V1 * AN - V1 * AN
    ...
    if Q > 0.5:
        ...
        for j in range(0, countOfPoints):
            ...
            test_responses[i, j] += random.random() * 2 * V1 * AN - V1 * AN
            max_response = max(max_response, random.random() * 2 * V1 * AN - V1 * AN)
            min_response = min(min_response, random.random() * 2 * V1 * AN - V1 * AN)

print(max_response)
print(min_response)
pyplot.plot(test_responses[numberOfGraphForShowing, :], "o-")
pyplot.show()
...
print("Percent of errors: ", countOf(abs(score - test_responses_type), 2) / len(score))
