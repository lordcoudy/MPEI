        ...
        try
          ...
          try
            try
              ...
              else if n>10 then writeln(fout,'Введено n больше 10')
              else if m>10 then writeln(fout,'Введено m больше 10')
              else if (m<1) then writeln(fout,'Введено m меньше 1')
              else
              begin
//              Ввод
                for i := 1 to n do
...
//              Ответ
                if k = 0 then
                  writeln(fout, 'Нет такой строки!')
                else
                  writeln(fout, 'Номер подходящей строки = ', k:2);
              end;
            except
              Err:=3;
            end;
    ...
    case Err of
    0: writeln('Success');
    1: Writeln('Ошибка открытия файла fin.txt ', paramstr(1));
    2: Writeln('Ошибка открытия/создания файла fout.txt', paramstr(2));
