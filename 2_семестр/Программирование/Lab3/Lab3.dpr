...
begin
  ...
  begin
    while ListC<>nil do
    begin
      if ListC.info <> 0 then ListTMP := ListC;         // Здесь доступен текущий элемент и информация в нем ListC^.info
      ListC:=ListC^.next;
    end;
    ListC:=NachaloSpiska;
    ...
    writeln('Номер искомого числа = ', n);
  end
  else writeln('Нет ненулевого элемента');
...
begin
  ...
  ListC:=KonecSpiska;
  writeln(#10#13'В обратном порядке:');
  while ListC<>nil do
  begin
    writeln(ListC^.info:5:1);
    ListC:=ListC^.prev;
