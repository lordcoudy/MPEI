...
org  offset_interupt				; Считываем состояние конечных выключателей каждое прерывание
	...
	jz tact1
	test al, 01011010b	; В такт 2, 4, 6
	jz tact2
	test al, 01101010b	; В такт 3
	jz farjmp1
	test al, 10100101b	; В такт 5
	jz farjmp2
...
tact1:
	...
	mov dx, 378h					; 378h - регистр данных 
	in ax, dx
	or ax, 00000011b				; Индикация по циклограмме tact1
	out dx, ax
...
tact2:	
	...
	mov dx, 378h					; 378h - регистр данных 
	in ax, dx
	or ax, 00000111b				; Индикация по циклограмме tact2
	out dx, ax
...
tact3:
	mov dx, 378h					; 378h - регистр данных 
	in ax, dx
	or ax, 00000011b				; Индикация по циклограмме tact3
	out dx, ax
...
tact4:
	mov al,my_mem
	and al, 00100100b				; Проверка, были ли мы уже в этом такте
	test al,11011011b
	jz tact5

	mov dx, 378h					; 378h - регистр данных 
	in ax, dx
	and ax, 00001011b				; Индикация по циклограмме tact4
	out dx, ax
	...
	and ax, 00010001b				; Подаем IRQ (4ый бит) и STROBE (0ой бит)
	out dx, ax
	or my_mem,00100100b				; Поднимаем соответствующий бит флага
	jmp exit

tact5:
	mov dx, 378h					; 378h - регистр данных 
	in ax, dx
	and ax, 00000011b				; Индикация по циклограмме tact5
...
tact6:
	mov dx, 378h					; 378h - регистр данных 
	in ax, dx
	and ax, 00000010b				; Индикация по циклограмме tact6
	out dx, ax
